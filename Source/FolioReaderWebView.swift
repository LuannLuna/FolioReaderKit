...
open class FolioReaderWebView: WKWebView {
    ...
    init(frame: CGRect, readerContainer: FolioReaderContainer) {
        ...
        userContentController.add(scriptMessageHandler, name: "myBridge")
        
        let userScript = WKUserScript(source: Bundle.frameworkBundle().path(forResource: "Bridge", ofType: "js")!, injectionTime: .atDocumentEnd, forMainFrameOnly: true)
        let userScriptForCss = WKUserScript(source: Bundle.frameworkBundle().path(forResource: "Style", ofType: "css")!, injectionTime: .atDocumentEnd, forMainFrameOnly: true)
        userContentController.addUserScript(userScript)
        userContentController.addUserScript(userScriptForCss)
    ...
    @objc func share(_ sender: UIMenuController) {
        ...
        let shareImage = UIAlertAction(title: self.readerConfig.localizedShareImageQuote, style: .default, handler: { (action) -> Void in
            if self.isShare {
                self.js("getHighlightContent()") { textToShare, error in
                ...
                self.js("getSelectedText()") { textToShare, error in
                    if let textToShare = textToShare {
                        self.folioReader.readerCenter?.presentQuoteShare(textToShare)
    ...
    func remove(_ sender: UIMenuController?) {
        self.js("removeThisHighlight()") { removedId, error in
    ...
    @objc func highlight(_ sender: UIMenuController?) {
        self.js("highlightString('\(HighlightStyle.classForStyle(self.folioReader.currentHighlightStyle))')") { highlightAndReturn, error in
    ...
    @objc func highlightWithNote(_ sender: UIMenuController?) {
        ...
        self.js("highlightStringWithNote('\(HighlightStyle.classForStyle(self.folioReader.currentHighlightStyle))')") { highlightAndReturn, error in
            if let highlightAndReturn = highlightAndReturn {
                let jsonData = highlightAndReturn.data(using: String.Encoding.utf8)
    ...
    @objc func updateHighlightNote (_ sender: UIMenuController?) {
        ...
        self.js("getHighlightId()") { highlightId, error in
    ...
    @objc func define(_ sender: UIMenuController?) {
        ...
        self.js("getSelectedText()") { selectedText, error in
    ...
    func changeHighlightStyle(_ sender: UIMenuController?, style: HighlightStyle) {
        ...
        self.js("setHighlightStyle('\(HighlightStyle.classForStyle(style.rawValue))')") { updateId, error in
...
//        }
    ...
    @discardableResult open func js(_ script: String, completionHandler: @escaping (String?, Error?) -> Void) {
